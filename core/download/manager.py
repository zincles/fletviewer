...
class DownloadManager:
    ...
    def create_task(self, url: str, filename: str, *, tags: list[str] | None = None, tag_data: dict | None = None, headers: dict[str, str] | None = None) -> DownloadTask:
        ...
        task_dir = self.downloading_dir / task_id
    ...
    def retry_task(self, task_id: str) -> None:
        self.initialize()
        task = self.get_task(task_id)
        if task and task.status in {"failed", "cancelled", "completed"}:
    ...
    def _load_tasks_from_disk(self) -> None:
        with self.data_db.connect() as conn:
            rows = conn.execute("SELECT payload_json FROM download_tasks ORDER BY created_at DESC").fetchall()
        ...
        with self.data_db.connect() as conn:
            conn.execute(
                ...
                INSERT INTO download_tasks(
                    id, payload_json, status, provider, source_url, title, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    ...
                    task.tag_data.get("gallery_details", {}).get("title", task.filename),
                    task.created_at,
                    task.updated_at,
                ),
            )
