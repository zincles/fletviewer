...
| 已完成 | 可选内嵌调试 WebUI | 无 Node.js 的服务端渲染 Dashboard、EH 主页/详情/缩略图/按需单页 reader、Booru 搜索/详情、Pixiv 详情、operation 列表/详情、Fetch/取消和结果图片；reader 提供上下页、页码跳转、access key、边界校验和 operation/resource 展示；Dashboard 和活动 operation 页面按状态自动刷新，listener 与 WebUI 可分别开关 |
...
| 进行中 | Provider 纵向迁移 | EH、Danbooru、Gelbooru、Pixiv 的查询/翻页和 provider-scoped 收藏搜索已接入 redb schema v3、Core/API/WebUI；Python 已绑定的 17 个 Booru 均已贯通搜索/详情/original fetch，Booru original 与 Pixiv 指定页均已支持复用 `ImageService` 的持久单图下载、节流进度、终态重试、安全删除、调试 WebUI 管理及有界持久队列；EH Archive 与图片任务已提供统一前端安全 `DownloadTaskView` 列表/detail HTTP 契约及 server 集成 smoke。Flet 仍使用 Python Core，正式切换前不并行连接 Rust 写同一存储；下一步补统一 command facade 与平台 transport 决策 |
