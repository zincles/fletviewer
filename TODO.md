...
| 进行中 | Provider 纵向迁移 | EH、Danbooru、Gelbooru、Pixiv 的查询/翻页和 provider-scoped 收藏搜索已接入 redb schema v3、Core/API/WebUI；Python 已绑定的 17 个 Booru 均已贯通搜索/详情/original fetch，前 12 个支持标签补全；有 MD5 时校验摘要，无 MD5 的 Philomena/Paheal 使用稳定 `ResourceKey` alias 并在重启后从内容缓存命中；后续实现 Pixiv/Booru 正式持久单图下载 |
