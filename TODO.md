...
| 进行中 | Provider 纵向迁移 | EH、Danbooru、Gelbooru、Pixiv 的查询/翻页和 provider-scoped 收藏搜索已接入 redb schema v3、Core/API/WebUI；Python 已绑定的 17 个 Booru 均已贯通搜索/详情/original fetch，Booru original 与 Pixiv 指定页均已支持复用 `ImageService` 的持久单图下载及终态重试/安全删除；有 MD5 时校验摘要，无 MD5 时使用稳定 `ResourceKey` alias 并在重启后从内容缓存命中；下一步补齐正式图片下载任务的进度语义 |
