...
| 进行中 | Provider 纵向迁移 | EH、Danbooru、Gelbooru、Pixiv 的查询/翻页和 provider-scoped 收藏搜索已接入 redb schema v3、Core/API/WebUI；Python 已绑定的 17 个 Booru 均已贯通搜索/详情/original fetch，Booru original 与 Pixiv 指定页均已支持复用 `ImageService` 的持久单图下载、节流进度、终态重试、安全删除、调试 WebUI 管理及有界持久队列；Runtime 公开 active/queued/terminal 统计和配置上限；下一步将图片任务统计接入下载页 DTO 并做平台 smoke |
