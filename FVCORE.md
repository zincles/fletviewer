...
| 已完成 | 配置、错误、ID 与 Runtime snapshot | executable 同级必需 `config.json`、无自定义运行时配置路径、`run`/`web` 模式、`check-config`、不覆盖式 `create-config`、严格 JSON、稳定错误、UUID v7 Runtime ID、revision 和生命周期 |
...
| 下一步 | 本地画廊导出语义 | 设计有界 stream/resource handle、任务占用和 Windows/Linux/Android/server 文件交换；Web 下载不得暴露或伪装服务器 Path |
