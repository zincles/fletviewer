...
- 本轮只迁移 EH 官方 Archive 下载、Pixiv 单图和 Booru 单图；ZIP/CBZ 索引、单页读取、自建 CBZ、本地画廊和任何前端接入均暂停。
...
- 当前 Python `core/` 继续作为已跑通产品实现、fixture 来源和行为参考；迁移采用纵向能力，不逐行翻译，也不允许 Python/Rust 双写真实存储。
...
独立程序读取并验证配置、获得存储实例锁、构造唯一 Runtime、恢复任务、启动控制面并优雅关闭。具体本地/远程控制传输在核心方法稳定后实现，但不能反向污染 Provider、缓存或下载模块。
...
Python 已有 Facade/DTO、Runtime 和 UI-independent 边界继续保留，但 Rust 不照搬 Python 线程池、可变任务对象、base64 DTO 或 import 副作用装配。
...
- ZIP/CBZ 索引、member 读取、CRC、封面提取、本地画廊、自建 CBZ 和 Archive 阅读。
...
- WASM 构建。
- EH 逐页批量下载、Pixiv 整本/ugoira、Booru tag 批量下载。
- 在 Rust 未达到验收标准前切换现有 Python + Flet 产品所有权。
...
- 在公开 Core 方法稳定后实现最小跨进程 command/query/event/resource transport。
- 本阶段仍不实现任何前端；传输只包装 Core 方法，不产生第二套业务模型。
...
- `fvcore` 单 crate 同时可 `cargo build --lib` 和构建可执行程序，不依赖 Python、Flet、Flutter、Bevy、浏览器或 Web framework。
...
1. 评估并选定覆盖 Windows/Linux/Android/server 的 async、HTTP、bytes、MD5、配置、ID、取消和测试依赖。
2. 为单 crate 的 library/executable 建立 format、clippy、test、doc、依赖审计和目标平台 compile CI。
3. 定义第一版 `CoreConfig`、`CoreError`、ID/revision、Command/Snapshot/Event/Resource 模型。
4. 实现可测试的 `CoreRuntime`/`CoreHandle`、生命周期、supervisor 和 fake operation。
5. 通过全部并发/取消基线后，再进入共享 session 和图像内容缓存。
