...
| 已完成 | Provider 限流与 Booru 查询闭环 | Generation 级并发/启动间隔；Danbooru JSON、Gelbooru JSON DAPI、参数化的 Safebooru/Rule34/TBIB/Xbooru/Hypnohub XML DAPI，以及 Yande.re/Konachan/Konachan.net/Lolibooru/Behoimi Moebooru JSON 搜索/详情/标签补全、fixture、稳定错误和 HTTP 路由；标签建议统一公开 tag/category/count 而保留四套协议解析；共 12 个 Provider，存在有效 Provider MD5 时 original 复用完整性 fetch 链路，缺失时明确拒绝未校验 fetch |
...
| 下一步 | Provider 纵向迁移 | EH、Danbooru、Gelbooru、Pixiv 查询均支持 provider-scoped 收藏搜索；12 个 Booru 已完成搜索/详情/标签补全并在有效 MD5 存在时支持 original fetch；下一步迁移 Python 已有 E621/E926，再迁移 Philomena/Derpibooru/Furbooru，并继续正式下载能力 |
