...
async fn dashboard(State(state): State<ControlState>) -> Response {
    ...
    let body = format!(
        concat!(
            ...
            "<tr><th>数据身份</th><td colspan=\"3\"><code>{}</code></td></tr>",
            "<tr><th>缓存身份</th><td colspan=\"3\"><code>{}</code></td></tr>",
            "<tr><th>下载身份</th><td colspan=\"3\"><code>{}</code></td></tr>",
            "<tr><th>临时域身份</th><td colspan=\"3\"><code>{}</code></td></tr></tbody></table></section>",
        ...
        snapshot.storage.cache_identity,
        snapshot.storage.downloads_identity,
        snapshot.storage.temp_identity,
        profile_rows,
        search,
        eh_form,
        pixiv_form,
...
async fn configuration(
    ...
    html_page(
        ...
        &format!(
            ...
            config.storage.data_identity,
            config.storage.cache_identity,
            config.storage.downloads_identity,
            config.storage.temp_identity,
        ),
        None,
    )
}
