...
- 新建的 `fvcore/` 是正在全面重写的唯一未来业务核心，必须保持纯 Rust；当前 Python `core/` 仅作为重写期间的只读行为和 fixture 参考，最终由 `fvcore` 完整替代并退出正式运行路径。
- `fvcore` 固定为单一 Cargo crate，同一 package 同时提供可嵌入 library 和可独立运行的 executable；内部实现 Provider、共享会话、图像、缓存、下载、任务和统一 Runtime。
- `fvcore` 的最终范围覆盖现有 Python Core 的全部 Provider、网络、会话、图像、缓存、下载、ZIP/CBZ、本地画廊、历史和存储能力；迁移按纵向能力分阶段交付，不等于永久裁剪范围。
- 标准 `fvcore` executable 始终编译集成 HTTP 控制面，可根据配置文件或命令行决定是否监听；同一 Core 也可作为 Rust library 嵌入其他程序。
...
| `core/` | 待替代的 Python Core；重写期间仅作为行为和 fixture 参考 |
